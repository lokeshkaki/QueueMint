...
aws ssm put-parameter \
  --name /<project-name>/dlq/features/llmClassificationEnabled \
  --value "true" \
  --type String

aws ssm put-parameter \
  --name /<project-name>/dlq/features/pagerdutyIntegrationEnabled \
  --value "true" \
  --type String
```
...
aws ssm put-parameter \
  --name /<project-name>/dlq/config/maxRetries \
  --value "3" \
  --type String
```
